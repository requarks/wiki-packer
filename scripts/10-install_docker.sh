...
docker create --name=watchtower --network=wikinet -v /var/run/docker.sock:/var/run/docker.sock containrrr/watchtower --cleanup --schedule="0 2 * * 6" --restart=unless-stopped wiki
